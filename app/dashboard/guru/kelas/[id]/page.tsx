...
type TabType = "materi" | "tugas" | "penilaian" | "orang";
...
export default function ClassDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const tabs: { id: TabType; label: string; icon: React.ReactNode }[] = [
    ...
    { id: "penilaian", label: "Penilaian", icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg> },
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="flex flex-col items-center">
          ...
          <p className="text-gray-500 font-medium">Memuat kelas...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      {/* ── Header Banner ── */}
      <div className={`relative bg-gradient-to-r ${classData.coverColor} pt-6 pb-24 px-4 sm:px-6 overflow-hidden`}>
        {/* Decorative elements */}
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col justify-center">
            ...
            <div className="grid grid-cols-3 gap-4 border-t border-gray-50 pt-4">
              <div>
                ...
                <p className="text-2xl font-extrabold text-gray-900">{classData.materials}</p>
              </div>
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Tugas</p>
                <p className="text-2xl font-extrabold text-gray-900">{classData.tasks}</p>
              </div>
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Anggota</p>
                <p className="text-2xl font-extrabold text-gray-900">{classData.students}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="space-y-4">
              {classData.schedule.length > 0 ? (
                classData.schedule.map((s: any, idx: number) => (
