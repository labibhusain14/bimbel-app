          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {(item as any).badge && sidebarOpen && <span className="ml-auto text-xs bg-purple-600 text-white px-1.5 py-0.5 rounded-full">{(item as any).badge}</span>}
                {(item as any).badge && !sidebarOpen && <span className="absolute top-1 right-1 w-2 h-2 bg-purple-600 rounded-full" />}
              </Link>
            );
          })}
        </nav>
              ...
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {(item as any).badge && <span className="ml-auto text-xs bg-purple-600 text-white px-2 py-0.5 rounded-full">{(item as any).badge}</span>}
                  </Link>
                );
              })}
            </nav>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className="bg-white border-b border-gray-100 px-4 sm:px-6 py-3 flex items-center gap-3 shrink-0">
          ...
          <div className="flex items-center gap-2 ml-auto">
            {isTeacher && (
              <div className="hidden sm:flex items-center justify-center w-8 h-8 bg-indigo-50 text-indigo-700 rounded-full mr-2" title="Mode Pengajar">
        ...
        {navItems.slice(0, 5).map((item) => {
          ...
          return (
            ...
            >
              ...
              {(item as any).badge && <span className="absolute top-0.5 right-1.5 w-2 h-2 bg-purple-600 rounded-full" />}
            </Link>
          );
        })}
      </nav>
