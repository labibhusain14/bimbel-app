...
// ── Color palettes ─────────────────────────────────────────────
const colorList = [
  { border: "border-violet-400", bg: "bg-violet-50", text: "text-violet-700", badge: "bg-violet-100 text-violet-700", dot: "bg-violet-500" },
  { border: "border-teal-400",   bg: "bg-teal-50",   text: "text-teal-700",   badge: "bg-teal-100 text-teal-700",   dot: "bg-teal-500"   },
  { border: "border-amber-400",  bg: "bg-amber-50",  text: "text-amber-700",  badge: "bg-amber-100 text-amber-700",  dot: "bg-amber-500"  },
  { border: "border-rose-400",   bg: "bg-rose-50",   text: "text-rose-700",   badge: "bg-rose-100 text-rose-700",   dot: "bg-rose-500"   },
  { border: "border-indigo-400", bg: "bg-indigo-50", text: "text-indigo-700", badge: "bg-indigo-100 text-indigo-700", dot: "bg-indigo-500" },
  { border: "border-emerald-400",bg: "bg-emerald-50",text: "text-emerald-700",badge: "bg-emerald-100 text-emerald-700",dot:"bg-emerald-500"},
];

// Assign color to each unique classroomId
const colorCache: Record<string, typeof colorList[0]> = {};
let colorIdx = 0;
function getColor(id: string) {
  if (!colorCache[id]) {
    colorCache[id] = colorList[colorIdx % colorList.length];
    colorIdx++;
  }
...
export default function JadwalPage() {
  ...
  const [scheduleByDay, setScheduleByDay] = useState<Record<number, ScheduleItem[]>>({
    0: [], 1: [], 2: [], 3: [], 4: [], 5: [], 6: [],
  });
  ...
  const schedule: ScheduleItem[] = selectedDayIdx >= 0 ? (scheduleByDay[selectedDayIdx] ?? []) : [];
    ...
    <div className="px-4 sm:px-6 py-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {loading ? "Memuat jadwal..." : `${totalSessions} sesi terjadwal minggu ini`}
          </p>
        </div>
        {!isCurrentWeek && (
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-4">
        ...
        <div className="grid grid-cols-7 gap-1">
          {weekDates.map((date, idx) => {
            ...
            const sessions   = scheduleByDay[idx] ?? [];
      ...
      <div className="flex items-center justify-between mb-3 px-1">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            {loading ? "Memuat..." : schedule.length === 0 ? "Tidak ada jadwal" : `${schedule.length} sesi`}
          </p>
        </div>
      ...
      {loading ? (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-14 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-sm text-gray-400">Memuat jadwal dari database...</p>
        </div>
      ) : schedule.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-14 flex flex-col items-center justify-center text-center">
        ...
        <div className="flex flex-col">
          {schedule.map((item, idx) => {
            ...
            const color    = getColor(item.classroomId);
                    ...
                    <div className={`relative bg-white rounded-xl border-l-4 ${color.border} shadow-sm hover:shadow-md transition-shadow p-4 border border-gray-100`}>
                      ...
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        <span className={`shrink-0 text-xs font-semibold px-2 py-0.5 rounded-full ${
                          item.type === "Online" ? "bg-blue-50 text-blue-600" : "bg-green-50 text-green-600"
                        }`}>
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-400">
                        ...
                        <span className={`font-medium ${color.text}`}>{duration}</span>
                      </div>
        ...
        <div className="grid grid-cols-7 gap-1.5">
          {weekDates.map((date, idx) => {
            ...
            return (
              ...
              >
                <span className={`text-[10px] font-semibold ${isToday ? "text-purple-600" : isSelected ? "text-purple-500" : "text-gray-400"}`}>
                  {DAY_SHORT[idx]}
                </span>
                {count > 0 ? (
                  <div className="flex flex-col items-center">
                    <span className={`text-base font-extrabold ${isSelected ? "text-purple-700" : "text-purple-500"}`}>{count}</span>
                    <span className={`text-[9px] font-medium ${isSelected ? "text-purple-400" : "text-gray-400"}`}>sesi</span>
                  </div>
                ) : (
                  <div className="flex flex-col items-center">
