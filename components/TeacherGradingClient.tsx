...
import { gradeSubmission } from "@/app/actions/kelas";
...
export default function TeacherGradingClient({ initialSubmissions }: { initialSubmissions: any[] }) {
  ...
  const handleGrade = async () => {
    ...
    if (res.success) {
      ...
      setSubmissions(prev => 
        prev.map(s => 
          s.id === selectedSub.id 
            ? { ...s, score: parseInt(score, 10), status: "graded" } 
            : s
        )
      );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
            <div className="p-6 border-b border-gray-100">
              ...
              <p className="text-sm text-gray-500 mt-1">{selectedSub.assignmentTitle} - {selectedSub.className}</p>
            </div>
